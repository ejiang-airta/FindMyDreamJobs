...
test.describe('Login & Navigation', () => {
  ...
  test('login-Test-1-Visit-login-page', async ({ page }) => {
  ...
  test('login-Test-2-Login-fails-incorrect-credentials', async ({ page }) => {
  ...
  test('login-Test-3-Login-succeeds', async ({ page }) => {
    await loginAsTestUser(page)
    await expect(page.getByText('Welcome back, test user1!')).toBeVisible({ timeout: 15000 })
  })

  test('login-Test-4-Navigate-to-Dashboard', async ({ page }) => {
    await loginAsTestUser(page)
  ...
  test('login-Test-7-Navigate-to-Optimize', async ({ page }) => {
    await loginAsTestUser(page)
  ...
  test('login-Test-12-Navigate-to-Home', async ({ page }) => {
    await loginAsTestUser(page)
    await page.getByRole('link', { name: 'Analyze' }).click()
